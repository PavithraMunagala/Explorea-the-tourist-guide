...
<%
    String originalHotelName = request.getParameter("OriginalHotelName");
    ...
    if (originalHotelName == null || originalHotelName.trim().isEmpty() ||
        newHotelName == null || newHotelName.trim().isEmpty() ||
        ...
        try {
            ...
            String checkSql = "SELECT COUNT(*) FROM Hotel WHERE HotelName = ?";
            pstmtCheck = conn.prepareStatement(checkSql);
            pstmtCheck.setString(1, newHotelName);
            ...
            if (nameExists && !originalHotelName.equals(newHotelName)) {
                out.println("<h2>Error: The new hotel name already exists. Choose a different name.</h2>");
            } else {
                // SQL to update both HotelName and Address
                String updateSql = "UPDATE Hotel SET HotelName = ?, Address = ? WHERE HotelName = ?";
                pstmtUpdate = conn.prepareStatement(updateSql);
                pstmtUpdate.setString(1, newHotelName);
                pstmtUpdate.setString(2, address);
                pstmtUpdate.setString(3, originalHotelName);

                int rowsAffected = pstmtUpdate.executeUpdate();
                if (rowsAffected > 0) {
                    response.sendRedirect("hotel.jsp"); // Redirect to the hotels list after updating
                } else {
