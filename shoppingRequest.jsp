...
<head>
    ...
    <title>Tourist Spots List</title>
    <style>
...
try {
    ...
    rs = stmt.executeQuery("SELECT * FROM suggestshoppingmalls LIMIT 50"); // Adjust to get only 20 records
    

    while (rs.next()) {
        String ShoppingMallsName = rs.getString("ShoppingMallsName");
        String Address = rs.getString("Address");
        String AdditionalInfo = rs.getString("AdditionalInfo");
%>
   <div class="hotel-item">
    <h3 class="hotel-name"><%=ShoppingMallsName %></h3>
    <p class="hotel-address"><%= Address %></p>
    <p class="hotel-address"><%= AdditionalInfo %></p>
    <div class="hotel-actions">
    <!-- Accept button form -->
    <form action="HotelAccept.jsp" method="post" style="display:inline;">
        <input type="hidden" name="ShoppingMallsName" value="<%= ShoppingMallsName %>">
        <input type="hidden" name="Address" value="<%= Address %>">
        <button type="submit" class="action-btn feedback-btn">Accept</button>
    </form>

    <!-- Decline button form (already exists) -->
    <form action="HotelDecline.jsp" method="post" style="display:inline;">
        <input type="hidden" name="ShoppingMallsName" value="<%= ShoppingMallsName %>">
        <button type="submit" class="update-btn">Decline</button>
    </form>
</div>
</div>
...
<%
    ...
    out.println("Unable to load Tourist Spots information at the moment. Error: " + e.getMessage());
    e.printStackTrace();
} finally {
